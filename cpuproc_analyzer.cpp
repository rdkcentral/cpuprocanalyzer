 ...
 */
void ReadProcessName(char* procName)
{
    char ch;
    int i = 0, j = 0;
    for (j = 0 ; procName[j] != '(' ; j++);
    for ( ; i<strlen(procName) ;)
    {
        ch = procName[j+1];
        if(ch == ')') break;
...
/**
 * @brief This function reads the Process Command Line Output from /proc/cmdline and removes extra
    characters.
 *
 * @param[in] fp         File Pointer
 * @param[in] str        String
...
void ReadSkippingRandomChar(FILE* fp, char* str)
{
    ...
    while (1) 
    {
      ch = fgetc(fp);
      if (ch == EOF) 
        break; // check for end of file and break the loop
      if (ch == 0) 
        ch = ' '; // replace null character with space
      str[i] = (char)ch;
      ++i;
    }
    str[i] = '\0';
...
char* GetValuesFromFile (char *fname, char *searchStr, char *strValue, unsigned int strValueLen)
{
    ...
    while (fgets(buf1,BUFF_SIZE_64,fp))
    {
        if (strstr(buf1, searchStr))
        {
            ...
            snprintf(srch, BUFF_SIZE_64, "%s%%s", searchStr);//CID:161123-Calling risky function
    ...
    if(fp1)
    {
        for(int i=0; i<arrlen; i++)
        {
            ...
            fprintf(fp1, "%s : %s\n", json_object_get_string(name_obj),
                    json_object_get_string(enable_obj));
        }
        fclose(fp1);
    }
    else
...
void GetNumOfClientsConnected(unsigned int *cliCount)
{
    ...
    fp = popen("dmcli eRT getv Device.Hosts.HostNumberOfEntries | grep value | awk '{print $5}'",
               "r");
    if(fp)
    {
        if(fgets(buf, 8, fp))
...
void ReadDeviceName()
{
    ...
    char name[100]                 = "\0";
      ...
      GetValuesFromFile ((char*)"/etc/device.properties", (char*)"MANUFACTURE=", mfgname, sizeof(mfgname));

      if (strncmp(mfgname,"UNKNOWN",strlen(mfgname)))
      {
          GetValuesFromFile ((char*)"/etc/device.properties", (char*)"MFG_NAME=", mfgname, sizeof(mfgname));
      }

      memset(devicename,0,BUFF_SIZE_16);
      GetValuesFromFile ((char*)"/etc/device.properties", (char*)"BOX_TYPE=", devicename, sizeof(devicename));
    #elif defined PROCANALYZER_EXTENDER
      ...
      GetValuesFromFile ((char*)"/etc/device.properties", (char*)"MFG_NAME=", mfgname, sizeof(mfgname));
      memset(devicename,0,BUFF_SIZE_16);
      GetValuesFromFile ((char*)"/etc/device.properties", (char*)"DEVICE_NAME=", devicename, sizeof(devicename));
    #endif
    ...
    GetValuesFromFile ((char*)"/version.txt", (char*)"imagename:", buildname, sizeof(buildname));
    ...
    else
    {
        ...
        RDK_LOG(RDK_LOG_INFO, "LOG.RDK.CPUPROCANALYZER", "Device name %s and Build name"
 ...
 * - The Kernel Flags word of the Process, Major Faults, Minor Faults, Nice value, Number of
 ...
 */
    void ReadProcStat(FILE* fp_procStat, int is_dynamic)
    {
        if (!is_dynamic)
        {
            //CID:337523-Unchecked return value
            int result = fscanf(fp_procStat, "%d %s %c %d %d %d %d %d %u %lu %lu %lu %lu %lu %lu %ld %ld %ld %ld %ld %ld %llu %lu %ld %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %d %d %u %u %llu %lu %ld",
                                &d_pid, s_comm, &c_state, &d_ppid, &d_pgrp, &d_session, &d_tty_nr,
                                &d_tpgid, &u_flags, &lu_minflt, &lu_cminflt, &lu_majflt, &lu_cmajflt,
                                &lu_utime, &lu_stime, &ld_cutime, &ld_cstime, &ld_priority, &ld_nice,
                                &ld_num_threads, &ld_itrealvalue, &llu_starttime, &lu_vsize, &ld_rss,
                                &lu_rsslim, &lu_startcode, &lu_endcode, &lu_startstack, &lu_kstkesp,
                                &lu_kstkeip, &lu_signal, &lu_blocked, &lu_sigignore, &lu_sigcatch,
                                &lu_wchan, &lu_nswap, &lu_cnswap, &d_exit_signal, &d_processor,
                                &u_rt_priority, &u_policy, &llu_delayacct_blkio_ticks, &lu_guest_time,
                                &ld_cguest_time);
            if(result == EOF)
            {
                    RDK_LOG(RDK_LOG_ERROR, "LOG.RDK.CPUPROCANALYZER","End of file reached.\n");
            }
            ReadProcessName(s_comm);
            RDK_LOG(RDK_LOG_TRACE1, "LOG.RDK.CPUPROCANALYZER","%s(%d): %d %s %c %d %d %d %d %d %u %lu %lu %lu %lu %lu %lu %ld %ld %ld %ld %ld %ld %llu %lu %ld %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %d %d %u %u %llu %lu %ld\n",
                                     __func__, __LINE__, d_pid, s_comm, c_state, d_ppid,
                                     d_pgrp, d_session, d_tty_nr, d_tpgid, u_flags, lu_minflt,
                                     lu_cminflt, lu_majflt, lu_cmajflt, lu_utime, lu_stime, ld_cutime,
                                     ld_cstime, ld_priority, ld_nice, ld_num_threads, ld_itrealvalue,
                                     llu_starttime, lu_vsize, ld_rss, lu_rsslim, lu_startcode, 
                                     lu_endcode, lu_startstack, lu_kstkesp, lu_kstkeip, lu_signal,
                                     lu_blocked, lu_sigignore, lu_sigcatch, lu_wchan, lu_nswap,
                                     lu_cnswap, d_exit_signal, d_processor, u_rt_priority, u_policy,
                                     llu_delayacct_blkio_ticks, lu_guest_time, ld_cguest_time);
        }
        else
        {
            int result = fscanf(fp_procStat, "%d %s %c %d %d %d %d %d %u %lu %lu %lu %lu %lu %lu %ld %ld %ld %ld %ld %ld %llu %lu %ld %lu",
                                &d_pid, s_comm, &c_state, &d_ppid, &d_pgrp, &d_session, &d_tty_nr,
                                &d_tpgid, &u_flags, &lu_minflt, &lu_cminflt, &lu_majflt, &lu_cmajflt,
                                &lu_utime, &lu_stime, &ld_cutime, &ld_cstime, &ld_priority, &ld_nice,
                                &ld_num_threads, &ld_itrealvalue, &llu_starttime, &lu_vsize, &ld_rss,
            ...
            ReadProcessName(s_comm);
            RDK_LOG(RDK_LOG_TRACE1, "LOG.RDK.CPUPROCANALYZER","%s(%d): %d %s %c %d %d %d %d %d %u %lu %lu %lu %lu %lu %lu %ld %ld %ld %ld %ld %ld %llu %lu %ld %lu\n",
                                    __func__, __LINE__, d_pid, s_comm, c_state, d_ppid, d_pgrp,
                                    d_session, d_tty_nr, d_tpgid, u_flags, lu_minflt, lu_cminflt,
                                    lu_majflt, lu_cmajflt, lu_utime, lu_stime, ld_cutime, ld_cstime,
                                    ld_priority, ld_nice, ld_num_threads, ld_itrealvalue,
                                    llu_starttime, lu_vsize, ld_rss, lu_rsslim);
        }
    }

/**
 * @brief This function constructs a file name based on parameters such as -
         output Directory Name,PID value,s_comm data,PID value,s_comm data. 
 ...
 */
    void OutFilename(char* outProcFilename,int is_dynamic)
    {
        if(is_dynamic == 1)
        {
            sprintf(outProcFilename, "%s%d_%s/%d_%s.data", outputDynamicDir.c_str(), d_pid,
                    s_comm,d_pid,s_comm);
        }
        else
        {
            sprintf(outProcFilename, "%s%d_%s/%d_%s.data", outputDir.c_str(), d_pid, s_comm,d_pid,
                    s_comm);
        }
    }
...
/**
 * @brief This function constructs a file name based on parameters such as -
          output Directory Name,PPID value,Parent process Name,PID value,s_comm data. 
 ...
 */
    void OutFilename(char* outProcFilename, int ppid, const char* pname)
    {
       //CID:348793-DC.string Buffer
        char tmp_string[1024] = {0};
        snprintf(outProcFilename, sizeof(tmp_string), "%s%d_%s/threads/%d_%s.data", outputDir.c_str(), ppid, pname,
                d_pid, s_comm);
    }
 ...
 * @param[in]  param         It can be parameter like Virtual Memory Size, Resident Set Size
                             and Size Of Stack.
 */
 ...
 * - The value after the slash is the number of kernel scheduling entities that currently
     exist on the system.
...
void GetLoadAverage(float* loadavg)
{
    ...
    {
        RDK_LOG(RDK_LOG_ERROR,"LOG.RDK.CPUPROCANALYZER","%s(%d): ERROR reading file: /proc/loadavg",
                                __func__, __LINE__);
    }
    else
    {
        //CID:45102-Unchecked return value from library
        if(fscanf(fp, "%f", loadavg) != 1)
        {
           RDK_LOG(RDK_LOG_ERROR,"LOG.RDK.CPUPROCANALYZER","%s(%d): ERROR reading load average.",
                                __func__, __LINE__);
        }
        fclose(fp);
    }
 ...
 *
 * - It reports statistics about memory usage on the system and gives information like
     Total usable RAM and Free Memory.
 ...
 *
 * - Idle time is the amount of time the CPU was not busy or otherwise,
     the amount of time it executed the System Idle process.
...
void GetUsedPercent(float* UsedPercent)
{
    char stat_buf[320]={0};
    unsigned long u, n, s, i, w, x, y, z;
    stCPUInfo currentCPUInfo;
    ...
    if(fp)
    {
        fgets(stat_buf, sizeof(stat_buf) - 1, fp);
        if(stat_buf[0] != '\0')
        {
            sscanf(stat_buf, "cpu %lu %lu %lu %lu %lu %lu %lu %lu", &u, &n, &s, &i, &w, &x, &y, &z);
            currentCPUInfo.total = u + s + i + w + x + y + z;
            currentCPUInfo.active = currentCPUInfo.total-i;
            *UsedPercent = (float)(currentCPUInfo.active - prevCPUInfo.active)/(currentCPUInfo.total - prevCPUInfo.total)*(100);
            prevCPUInfo.total = currentCPUInfo.total;
            prevCPUInfo.active = currentCPUInfo.active;
        }
        fclose(fp);
    }
}
...

{
    ...
    {
        //CID:280112-Unchecked return value from library
        if(fscanf(fp, "%d" , FDCount) != 1)
        {
            RDK_LOG(RDK_LOG_ERROR, "LOG.RDK.CPUPROCANALYZER","Error reading FD Count\n");
        }
        pclose(fp);
    }
}
...

{
    ...
    {
        //CID:280095-Unchecked return value from library
        if(fscanf(fp, "%d" , FDCountSystem) != 1)
        {
                RDK_LOG(RDK_LOG_ERROR, "LOG.RDK.CPUPROCANALYZER","Error Reading FD Count System Level\n");
        }
        fclose(fp);
    }
}
 ...
 *
 * - Information such as Process Name, PID of the parent, Virtual memory size,
     Resident set size, Size of Stack.
 *
 ...
 */
//CID:280113-Parse Warning
int LogProcData(stProcData* procData, int ppid=0, const char* pname="",int is_dynamic=0, bool telemetryOnly=0)
{
    char tmp_string[1024] = {0};
    ...
    if(ppid != 0)
    {
      snprintf(tmp_string, sizeof(tmp_string), "/proc/%d/task/%d/stat", ppid, procData->d_pid);
    }
    else
    {
      snprintf(tmp_string, sizeof(tmp_string), "/proc/%d/stat", procData->d_pid);
    }

    fp_stat = fopen(tmp_string, "r");
    if(fp_stat)
    {
        procData->ReadProcStat(fp_stat, is_dynamic);
        fclose(fp_stat);
    }
    else
    {
    ...
    if (!is_dynamic)
    {
        ...
        {
           //CID:337483,337480,337476-Parse Warning
           ...
           GetMemParams(tmp_string, &vmStack, const_cast<char*>(VmStk));
           GetMemParams(tmp_string, &vmSize, const_cast<char*>(VmSize));
           GetMemParams(tmp_string, &vmRSS, const_cast<char*>(VmRSS));
        }
    ...
    */
    snprintf(tmp_string, sizeof(tmp_string), "/proc/%d/cmdline", procData->d_pid);
    ...
    if(fp_cmd)
    {
        ...
        {
            snprintf(tmp_string, sizeof(tmp_string), "/proc/%d/comm", procData->d_pid);
            FILE* fp_cmd = fopen(tmp_string, "r");
            if(fp_cmd)
            {
                fgets(tmp_string, sizeof(tmp_string), fp_cmd);
    ...
    if (ppid != 0)
    {
        //CID:280115-Calling risky function
        snprintf(tmp_string, sizeof(tmp_string), "%s/%d_%s/threads/", outputDir.c_str(), ppid, pname);
    }
    else
    {
        if(is_dynamic  == 0)
        {
            snprintf(tmp_string, sizeof(tmp_string), "%s%d_%s", outputDir.c_str(), procData->d_pid, procData->s_comm);
        }
        else
        {
            snprintf(tmp_string, sizeof(tmp_string), "%s%d_%s", outputDynamicDir.c_str(), procData->d_pid, procData->s_comm);
        }
    }

    //CID:280101-Unchecked return value
    if(mkdir(tmp_string, S_IRWXU | S_IRWXG | S_IRWXO) != 0)
    {
        if(errno != EEXIST)
        {
            RDK_LOG(RDK_LOG_ERROR, "LOG.RDK.CPUPROCANALYZER","Failed to create %s directory\n", tmp_string);
        }
    }
    (ppid != 0) ? procData->OutFilename(tmp_string, ppid, const_cast<char*>(pname)) : procData->OutFilename(tmp_string,is_dynamic);
    ...
    if(fp_dataOut)
    {
        ...
        if(!is_dynamic)
        {
            ...
            status = prevData[procData->d_pid].status;
        }
        else
        {
            timeDiff_usec = currentTime_usec;
            if(timeDiff_usec == 0) timeDiff_usec = 1;
            cpuUseRaise = 100*(currentTotalUsedCPUTime)/(sysconf(_SC_CLK_TCK)*timeDiff_usec/1000000);
            cpuUseRaise_User = 100*(currentUserUsedCPUTime)/(sysconf(_SC_CLK_TCK)*timeDiff_usec/1000000);
            cpuUseRaise_System = 100*(currentSystemUsedCPUTime)/(sysconf(_SC_CLK_TCK)*timeDiff_usec/1000000);
            if(ppid == 0)
                RDK_LOG(RDK_LOG_TRACE1, "LOG.RDK.CPUPROCANALYZER", "%s(%d): %d %0.2lf %0.2lf %0.2lf %lu %ld %0.2lf\n", __func__, __LINE__, procData->d_pid, cpuUseRaise, cpuUseRaise_User, cpuUseRaise_System, currentTotalUsedCPUTime, sysconf(_SC_CLK_TCK), timeDiff_usec);
            majorFaultsRaise = (currentTotalMajorFaultsRaised);
        }

        if( (!is_dynamic) && (status != 1) )
        {
            snprintf(tmp_string, sizeof(tmp_string), "/proc/%d/cmdline", procData->d_pid);
            FILE* fp_cmd = fopen(tmp_string, "r");
            if(fp_cmd)
            {
                ...
                if(telemetryOnly == false)
                {
                    ...
                    if(PROC_MASK)
                    {
                        ...
                        {
                           //CID:280106-Invalid printf format string
                    ...
                    {
                       //CID:280110-Invalid printf format string
       ...
       if(is_dynamic == 1)
        {
            ...
            else
            {
                ...
                {
                    fprintf(fp_dataOut, "El-Time\tTimeStamp\t\tCPU%%\tCPU%%:U\tCPU%%:S\tMjrFlts\tVmSize\tVmRSS\tVmStk\tThreadCount\tFDCount\n");
                    fprintf(fp_dataOut, "%ld\t%s\t%0.2lf\t%0.2lf\t%0.2lf\t%d\t%ld\t%ld\t%ld\t%ld\t\t%d\t\n", totalTimeElapsed_sec, GetCurTimeStamp(), cpuUseRaise, cpuUseRaise_User, cpuUseRaise_System, procData->lu_majflt, procData->lu_vsize, procData->ld_rss, vmStack, ((procData->ld_num_threads)-1), FDCount);                    
/*              if(PROC_MASK)
                {
                    fprintf(fp_dataOut, "%ld\t%s", totalTimeElapsed_sec, GetCurTimeStamp());
                    if(PROC_MASK & CPU_MASK)
                    {
                      fprintf(fp_dataOut, "\t%0.2lf\t%0.2lf\t%0.2lf\t%d", cpuUseRaise, cpuUseRaise_User, cpuUseRaise_System, procData->lu_majflt);
                      RDK_LOG(RDK_LOG_TRACE1, "LOG.RDK.CPUPROCANALYZER", "%s(%d): %0.2lf %0.2lf %0.2lf %d \n", __func__, __LINE__, cpuUseRaise, cpuUseRaise_User, cpuUseRaise_System, procData->lu_majflt);
                    }
                    if(PROC_MASK & MEMORY_MASK)
                    {
                       fprintf(fp_dataOut, "\t%ld\t%ld\t%ld", procData->lu_vsize, procData->ld_rss, vmStack);
                       RDK_LOG(RDK_LOG_TRACE1, "LOG.RDK.CPUPROCANALYZER", "%s(%d): %ld %ld %ld \n", __func__, __LINE__, procData->lu_vsize, procData->ld_rss, vmStack);
                    }
                    if(PROC_MASK & THREADCOUNT_MASK)
                    {
                ...
                {
                   fprintf(fp_dataOut, "%ld\t%s\t%0.2lf\t%0.2lf\t%0.2lf\t%d\t%ld\t%ld\t%ld\t%ld\t\t%d\t\n", totalTimeElapsed_sec, GetCurTimeStamp(), cpuUseRaise, cpuUseRaise_User, cpuUseRaise_System, procData->lu_majflt, procData->lu_vsize, procData->ld_rss, vmStack, ((procData->ld_num_threads)-1), FDCount);
                   RDK_LOG(RDK_LOG_TRACE1, "LOG.RDK.CPUPROCANALYZER", "%s(%d): %0.2lf %0.2lf %0.2lf %d %ld %ld %ld %ld %d \n", __func__, __LINE__, cpuUseRaise, cpuUseRaise_User, cpuUseRaise_System, procData->lu_majflt, procData->lu_vsize, procData->ld_rss, vmStack, ((procData->ld_num_threads)-1), FDCount);
                } */
                }
            }
            fclose(f);
       ...
       if( (!is_dynamic) && status )
       {
            if(telemetryOnly == false)
            {
                if(PROC_MASK)
                {
                    ...
                    if(PROC_MASK & CPU_MASK)
                    {
                       fprintf(fp_dataOut, "\t%0.2lf\t%0.2lf\t%0.2lf\t%d", cpuUseRaise,
                               cpuUseRaise_User, cpuUseRaise_System, majorFaultsRaise);
                       RDK_LOG(RDK_LOG_TRACE1, "LOG.RDK.CPUPROCANALYZER", "%s(%d): %0.2lf %0.2lf %0.2lf %d \n",
                               __func__, __LINE__, cpuUseRaise, cpuUseRaise_User,
                               cpuUseRaise_System, majorFaultsRaise);
                    }
                    if(PROC_MASK & MEMORY_MASK)
                    {
                ...
                {
                   fprintf(fp_dataOut, "%ld\t%s\t%0.2lf\t%0.2lf\t%0.2lf\t%d\t%ld\t%ld\t%ld\t%ld\t\t%d\t\n",
                           totalTimeElapsed_sec, GetCurTimeStamp(), cpuUseRaise, cpuUseRaise_User,
                           cpuUseRaise_System, majorFaultsRaise, vmSize, vmRSS, vmStack,
                           ((procData->ld_num_threads)-1), FDCount);
                   RDK_LOG(RDK_LOG_TRACE1, "LOG.RDK.CPUPROCANALYZER", "%s(%d): %0.2lf %0.2lf %0.2lf %d %ld %ld %ld %ld %d \n",
                           __func__, __LINE__, cpuUseRaise, cpuUseRaise_User, cpuUseRaise_System,
                           majorFaultsRaise, vmSize, vmRSS, vmStack, ((procData->ld_num_threads)-1),
                           FDCount);
                }
           }
       }
       fclose(fp_dataOut);
         ...
         if(ppid == 0)
         {
             ...
             snprintf(telemetry_buf, sizeof(telemetry_buf), "%ld,%0.2lf,%d,%d", vmRSS, cpuUseRaise,
    ...
    while (!need_exit)
    {
       ...
       // Received an exec event from netlink socket
        case PROC_EVENT_EXEC:
            ...
            dProcData.d_pid = netlink_msg.process_event.event_data.exec.process_pid;
            LogProcData(&dProcData,0,"",1);
            break;
        case PROC_EVENT_UID:
            RDK_LOG(RDK_LOG_INFO,"LOG.RDK.CPUPROCANALYZER","UID_EVT: PID=%d TID=%d from %d to %d\n",
                    netlink_msg.process_event.event_data.id.process_pid,
                    netlink_msg.process_event.event_data.id.process_tgid,
                    netlink_msg.process_event.event_data.id.r.ruid,
                    netlink_msg.process_event.event_data.id.e.euid);
            break;
        case PROC_EVENT_GID:
            RDK_LOG(RDK_LOG_INFO,"LOG.RDK.CPUPROCANALYZER","GID_EVT: PID=%d TID=%d from %d to %d\n",
                    netlink_msg.process_event.event_data.id.process_pid,
                    netlink_msg.process_event.event_data.id.process_tgid,
                    netlink_msg.process_event.event_data.id.r.ruid,
                    netlink_msg.process_event.event_data.id.e.euid);
        ...
        case PROC_EVENT_COMM:
            RDK_LOG(RDK_LOG_INFO,"LOG.RDK.CPUPROCANALYZER","COMM_EVT: PID=%d TID=%d Name=%s\n",
                    netlink_msg.process_event.event_data.comm.process_pid,
                    netlink_msg.process_event.event_data.comm.process_tgid,
                    netlink_msg.process_event.event_data.comm.comm);
            break;
        case PROC_EVENT_COREDUMP:
            RDK_LOG(RDK_LOG_INFO,"LOG.RDK.CPUPROCANALYZER","CORE_DUMP_EVT: PID=%d TID=%d\n",
                    netlink_msg.process_event.event_data.coredump.process_pid,
                    netlink_msg.process_event.event_data.coredump.process_tgid);
            break;
        case PROC_EVENT_EXIT:
            RDK_LOG(RDK_LOG_INFO,"LOG.RDK.CPUPROCANALYZER","EXIT_EVT: PID=%d TID=%d, ExitCode=%d\n",
                    netlink_msg.process_event.event_data.exit.process_pid,
                    netlink_msg.process_event.event_data.exit.process_tgid,
                    netlink_msg.process_event.event_data.exit.exit_code);
            break;
        default:
...
static void *handle_proc_ev_thread(void *arg )
{
    ...
    close(netlink_sock);//CID:11849-Resource Leak
    return NULL;
}

//CID:161016-Structurally dead code
int checkifdigit(const char* ch, int size)
{
    int retid = 1;
...
bool CheckMemLimit (int itr, unsigned long memLimit)
{
    ...
    snprintf(tmp_filename, sizeof(tmp_filename), "du -s %s | awk '{print$1}'",outputDir.c_str());//CID:190392-Calling risky function
...
void AddToExclusionList(char* res)
{
    char *newline = strchr( res, '\n' );
    if ( newline )
    {
    ...
    while(token != NULL)
    {
        exclude_process_list.push_back(token);
        token = strtok(NULL, ",");
    }
}
 ...
 * @brief Main Function.
 *
 * Fetches information such as Load Average value, Used Memory value and Idle Percent value for
   all the Processes in the list from /opt/logs/cpuprocanalyzer/loadandmem.data.
 */
int main(int argc, char** argv)
{
    ...
    RDK_LOG(RDK_LOG_INFO, "LOG.RDK.CPUPROCANALYZER","Monitor All Process = %d, MemLimit = %ld\nDynamic = %d\ntelemetryOnly = %d\n", monitorAllProcess, memoryLimit, enableDynamic, telemetryOnly);
        ...
        memset(res,0,64);
        if((read_config_param("FEATURE.CPUPROCANALYZER.DynamicExclusionList",pEnvConfig,res)) != 0)
        {
            RDK_LOG(RDK_LOG_INFO, "LOG.RDK.CPUPROCANALYZER","DynamicExclusionList = %s\n", res);
            AddToExclusionList(res);
        }
        #endif

        //Clearing the contents of the output directory before running the tool
        snprintf(tmp_string, sizeof(tmp_string), "cd %s && rm -rf *", outputDir.c_str());
        system(tmp_string);
        ...
        {
          RDK_LOG(RDK_LOG_ERROR, "LOG.RDK.CPUPROCANALYZER","Failed to create outputDir.c_str() directory\n");
        }
        if(telemetryOnly == false)
        {
            if(mkdir(outputDynamicDir.c_str(), S_IRWXU | S_IRWXG | S_IRWXO) != 0)
            {
              RDK_LOG(RDK_LOG_ERROR, "LOG.RDK.CPUPROCANALYZER","Failed to create outputDynammic.c_str() directory\n");
            }
    ...
    while (iteration++, !terminate && CheckMemLimit (iteration,memoryLimit))
    {
        //Capture Load Average value
        FILE *fp;
        if(telemetryOnly == false)
        ...
        else
        {
            ...
            RDK_LOG(RDK_LOG_INFO, "LOG.RDK.CPUPROCANALYZER", "%s(%d): Load: %0.2f Mem: %ld \
                                        ...
                                        __func__, __LINE__, loadAvg, usedMemory, UsedPercent,
        ...
        if(fp)
        {
            if(firstIter)
            {
                ...
                {
                     //CID:280091:Invalid printf format string
            ...
            else
            {
                fprintf(fp, "%s\t%0.2f\t%ld\t%0.2f\t%d\t\t%d\n", GetCurTimeStamp(),
                             loadAvg, usedMemory, UsedPercent, FDCountSystem, cliCount);
            }
            fclose(fp);
        }
        else
        ...
        else if(access(PROCESSES_LIST_PATH, F_OK) != -1)
        {
           ...
           //Read from /opt/processes.list or /nvram/processes.list
              ...
              if (fp)
              {
                 ...
                 {
                    snprintf(tmpstring1, sizeof(tmpstring1), "pidof %s", buf1);//CID:47195-Calling risky function
           ...
           else
           {
               if(telemetryOnly == false)
               {
                   RDK_LOG(RDK_LOG_ERROR,"LOG.RDK.CPUPROCANALYZER"," Error opening the file: %s",
                                         ps_filename.c_str());
                   continue;
               }
           }
        ...
        if(!monitorSysLevel)
        {
           ...
           if(fp_selectedps)
           {
               ...
               while(it != prevData.end())
               {
                   ...
                   {
                       RDK_LOG(RDK_LOG_INFO,"LOG.RDK.CPUPROCANALYZER", "Set status to 0, PID = %d\n\n",
                                             it->first);
                       preData.status = 0;
                   }
                   //Removing-CID:161012-Using invalid iterator- Confirmed that the change is intentional
                   ++it;
               }
           }
           else
